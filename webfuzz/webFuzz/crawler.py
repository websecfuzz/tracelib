...
class Crawler:
    ...
    def __add__(self, links: Set[Node]):
        ...
        allowed_links = set(l for l in uniq_links if not self._should_block(l))
        self._crawler_unseen = self._crawler_unseen | allowed_links

        logger.debug("New links found: %s", allowed_links)
