...
chmod 777 /coverage 2>/dev/null || log "warn: chmod 777 /coverage failed (bind-mount may not permit it)"
...
log "starting tracelib on port $TRACELIB_PORT with header $TRACELIB_HEADER"
...
[ -n "$TRACELIB_EXTRA" ] && log "tracelib flags:$TRACELIB_EXTRA"

tracelib --port "$TRACELIB_PORT" --header "$TRACELIB_HEADER" $TRACELIB_EXTRA &
TRACELIB_PID=$!

while kill -0 "$APP_PID" 2>/dev/null && kill -0 "$TRACELIB_PID" 2>/dev/null; do
    wait -n "$APP_PID" "$TRACELIB_PID" 2>/dev/null || true
done
log "app or tracelib exited — shutting down"
