...
while true; do
    ...
    cov_out="$($DC exec -T -w "$APP_DIR" "$SERVICE" php /tracelib-support/coverage_report.php 2>/dev/null || true)"
    ...
    n_bm=$(find "$BITMAPS" -maxdepth 1 -type f \( -name 'wf-*' -o -name 'req-*' -o -name 'tl-smoke-*' \) 2>/dev/null | wc -l)
